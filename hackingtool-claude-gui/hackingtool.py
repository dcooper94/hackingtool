...
from workspace import get_active_workspace, set_active_workspace, list_workspaces, set_scope, read_history
...
def build_menu():
    ...
    console.print(
        ...
        "[dim cyan]w[/dim cyan] [dim]workspace[/dim]  "
...
def workspace_manager():
    while True:
        ws = get_active_workspace()
        table = Table(title=f"Workspace Manager — active: {ws.name}", box=box.SIMPLE_HEAD, show_lines=True)
        table.add_column("No.", justify="center", style="bold cyan")
        table.add_column("Action", style="bold yellow")
        table.add_column("Details", style="white")
        table.add_row("1", "List workspaces", "Show known workspaces")
        table.add_row("2", "Switch/create workspace", "Create if missing, then make active")
        table.add_row("3", "Set scope", "Comma-separated CIDRs, IPs, domains, or wildcard domains")
        table.add_row("4", "Show recent history", "Last 20 commands for active workspace")
        table.add_row("99", "Back", "")
        console.print(table)
        raw = Prompt.ask("[bold cyan]workspace>[/bold cyan]", default="99").strip()
        if raw == "99" or not raw:
            return
        if raw == "1":
            t = Table(title="Workspaces", box=box.SIMPLE_HEAD)
            t.add_column("Name", style="bold cyan")
            t.add_column("Scope", overflow="fold")
            for item in list_workspaces():
                t.add_row(item.name, ", ".join(item.scope) if item.scope else "not set")
            console.print(t)
            Prompt.ask("[dim]Press Enter[/dim]", default="")
        elif raw == "2":
            name = Prompt.ask("[bold cyan]Workspace name[/bold cyan]", default=ws.name).strip()
            if name:
                new_ws = set_active_workspace(name)
                console.print(f"[success]Active workspace: {new_ws.name}[/success]")
            Prompt.ask("[dim]Press Enter[/dim]", default="")
        elif raw == "3":
            scope_raw = Prompt.ask("[bold cyan]Scope items, comma-separated[/bold cyan]", default=", ".join(ws.scope)).strip()
            items = [x.strip() for x in scope_raw.split(",") if x.strip()]
            updated = set_scope(items, ws)
            console.print(f"[success]Scope updated for {updated.name}: {', '.join(updated.scope) if updated.scope else 'not set'}[/success]")
            Prompt.ask("[dim]Press Enter[/dim]", default="")
        elif raw == "4":
            rows = read_history(limit=20, workspace=ws)
            ht = Table(title="Recent Workspace History", box=box.SIMPLE_HEAD, show_lines=True)
            ht.add_column("Time", style="dim")
            ht.add_column("Tool", style="bold yellow")
            ht.add_column("Exit", justify="center")
            ht.add_column("Command", overflow="fold")
            for row in rows:
                ht.add_row(row.get("timestamp", ""), row.get("tool", ""), str(row.get("returncode", "")), row.get("command", ""))
            console.print(ht)
            Prompt.ask("[dim]Press Enter[/dim]", default="")
...
def interact_menu():
    while True:
        try:
            ...
            if raw_lower in ("w", "workspace", "workspaces"):
                workspace_manager()
                continue
