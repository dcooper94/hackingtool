...
from workspace import get_active_workspace, create_result_dir, append_history, scope_check
...
class App(tk.Tk):
    ...
    def _page_main(self):
        ...
        scope_text = ", ".join(ws.scope) if ws.scope else "scope not set"
        ws_bar = tk.Frame(self._area, bg=PANEL, height=24)
        ws_bar.pack(fill=tk.X)
        tk.Label(ws_bar, text=f"Workspace: {ws.name} | {scope_text}", font=F(8), bg=PANEL, fg=DIM).pack(side=tk.LEFT, padx=8)
    ...
    def _page_terminal(self, commands: list[str], title: str = "Terminal"):
        ...
        def _runner():
            for raw_cmd in commands:
                ...
                ws = get_active_workspace()
                cfg = load_config()
                scope = scope_check(cmd, ws)
                if scope.get("out_of_scope"):
                    self.after(0, _write, f"Out-of-scope target(s): {', '.join(scope['out_of_scope'])}\n", "info")
                    if cfg.get("block_out_of_scope", False):
                        self.after(0, _write, "Command blocked by workspace scope policy.\n", "err")
                        continue
                result_dir = create_result_dir(title, 1, ws)
                stdout_log = result_dir / "stdout.log"
                metadata_file = result_dir / "metadata.json"
                command_file = result_dir / "command.txt"
                command_file.write_text(cmd + "\n", encoding="utf-8")
                self.after(0, _write, f"Artifacts: {result_dir}\n", "info")
                started = time.monotonic()
                stdout_fh = stdout_log.open("w", encoding="utf-8", errors="replace")
                ...
                try:
                    ...
                    if self._term_gen == my_gen:
                        ...
                        metadata = {
                            "tool": title, "command": cmd, "workspace": ws.name,
                            "returncode": rc, "duration_seconds": duration, "scope": scope,
                            "artifacts": {"stdout": str(stdout_log), "command": str(command_file)},
                        }
                        metadata_file.write_text(json.dumps(metadata, indent=2, sort_keys=True), encoding="utf-8")
                        append_history(metadata, ws)
